<template>
    ...
    <div class="container-fluid">
        ...
        <div class="botao">
            <div class="">
                <router-link to="/createDepartament"><i class='mdi mdi-plus btn btn-outline-primary mb-1'></i></router-link>
            </div>
        </div>
            ...
            <table class="table table-striped table-dark">
                ...
                <tbody style="text-align: center;">
                    <tr v-for="(item, i) in  data.dadosPaginados " :key="i">
                        ...
                        <td>{{ item.name }}</td>
                        <td>{{ item.acronym }}</td>
                        <td><button @click="updateDepartament(item.departamentId)"
                                class='mdi mdi-pencil btn btn-warning'></button>
                        </td>
                        <td><button type="button" data-bs-toggle="modal" data-bs-target="#exampleModal"
                                @click="softDelete(item.departamentId)" class='mdi mdi-delete btn btn-danger'></button>
                        </td>
                    </tr>
                </tbody>
            </table>
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination">
                        <li class="page-item" v-on:click="getPreviousPage()"><a class="page-link"
                                href="/#/departament">Previous</a>
                        </li>
                        <li v-for="pagina in totalPaginas()" v-bind:class="isActive(pagina)"
                            v-on:click="getDataPagina(pagina)" class="page-item"><a class="page-link"
                                href="/#/departament">{{
                                    pagina
                                }}</a>
                        </li>
                        <li class="page-item" v-on:click="getNextPage()"><a class="page-link" href="/#/departament">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" @click="deleteDepartament(password)" class="btn btn-danger">Eliminar</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        async softDelete(id) {
            localStorage.setItem('deleteDepartamentId', id)
        },

        async deleteDepartament(password) {
            const id = localStorage.getItem('deleteDepartamentId')

            await this.$axios.delete(`/departament/departamentDelete/${id}`,
                {
                    headers: {
        ...
        async updateDepartament(value) {
            localStorage.setItem("updateDepartamentId", value)
            this.$router.push('/createDepartament')
        }
    },
}
</script>
